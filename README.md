# 🌸 Kadim Sakura

> **"Gölgeler geçici, ama efsaneler sonsuzdur."**

![banner](https://github.com/user-attachments/assets/e397a8f9-f539-46f8-85f6-b9a8a69825a2)

> [!IMPORTANT]
> **🚧 Proje Durumu: Geliştirme Devam Ediyor (Post-Jam Development)**
> Bu proje, **StartGate GameJam 2025** kapsamında **48 saat** gibi kısıtlı bir sürede sıfırdan geliştirilmiştir.
>
> Kod yapısı ve mimari, "hızlı prototipleme" (MVP) odaklı olduğu için optimizasyon eksiklikleri veya "spaghetti code" içerebilir. 🍝
> * **Jam Versiyonu:** İlk 48 saatin çıktısıdır.
> * **Şu Anki Durum:** Yarışma sonrası (Post-Jam) güncellemeler, bug fix'ler ve iyileştirmelerle proje aktif olarak geliştirilmeye devam etmektedir.
---

## 📖 Hakkında (About)
**Kadim Sakura**, Startgate Game Jam 2025 için **48 saatte** geliştirilmiş, Low Poly atmosferik bir **Hack & Slash** oyunudur.

Oyuncular, ezeli rakibi **Sasaki Kojiro**'nun kötü ruhu tarafından ele geçirilmeye çalışan efsanevi samuray **Miyamoto Musashi**'yi kontrol eder. Amacımız: Dünyanın son yaşam kaynağı olan Kadim Sakura ağacını gölgelerden korumak ve kendi içimizdeki karanlığı yenmek.

---

## ✨ Özellikler (Features)
* **⚔️ Boss Skill Steal (Ruh Çalma):** Yendiğin her Boss'un yeteneği senin silahın olur. Düşmanının gücünü ona karşı kullan!
* **🌸 Atmosferik Low Poly Dünya:** Neon mor ve Sakura pembesiyle harmanlanmış, mistik bir görsel şölen.
* **👹 Mitolojik Düşmanlar:** Japon folkloründen esinlenen karanlık varlıklar.
* **📜 Manga Tarzı Anlatım:** Hikaye derinliğini artıran özel çizim sinematikler.

---

## 🎮 Kontroller (Controls)

| Tuş | Eylem |
| :---: | :--- |
| **W, A, S, D** | Hareket |
| **Sol Tık** | Saldırı (Attack) |
| **Sağ Tık** | Sav |
| **E** | Dash |
| **Q** | Işınlanma |

---

## 👥 Takım: The Samurai Jacks

Bu proje **The Samurai Jacks** takımı tarafından tutkuyla geliştirilmiştir.

| İsim | Rol & Unvan |
| :--- | :--- |
| [**Arda Anıl Morkoç**](https://www.linkedin.com/in/ardamorkoc/) | 🚀 Team Lead & Developer |
| [**Zehra Gürcan**](https://www.linkedin.com/in/zehra-g%C3%BCrcan-874a3625b/) | 💻 Core Developer |
| [**Yusuf Üveys Kaplan**](https://www.linkedin.com/in/yusuf-%C3%BCveys-kaplan/) | 🎨 Environment & UI Designer |
| [**Sudenaz Ulusay**](https://www.linkedin.com/in/sudenaz-ulusay-80b85a293/) | 🖌️ 2D Artist & UI/Environment |
| [**Adil Umut Tuncer**](https://www.linkedin.com/in/adilumut-tuncer/) | 🎵 Audio & SFX Designer |

---

## 🛠️ Kullanılan Teknolojiler & Kaynaklar
* **Oyun Motoru:** Unity 2022.3 (URP)
* **Dil:** C#
* **VCS:** GitHub
* **3D Model:** Karakter ve çevre modelleri projenin kısıtlı süresi nedeniyle AI destekli araçlar ve hazır assetler modifiye edilerek (Mixamo vb.) kullanılmıştır.
* **2D Art & UI:** Takımımız tarafından tasarlanmış, Manga çizimleri AI desteğiyle üretilip editlenmiştir.

---

## 📥 İndir ve Oyna (Download & Play)

### 🚀 Güncel Sürüm (Önerilen)
Hata düzeltmeleri ve iyileştirmeler içeren en güncel versiyonu **Itch.io** üzerinden indirebilirsiniz:

[![Itch.io İndir](https://static.itch.io/images/badge-color.svg)](https://ardamorkoc.itch.io/kadim-sakura)

### 🏛️ Orijinal Jam Sürümü (Arşiv)
StartGate GameJam'25 kapsamında **48. saatte teslim edilen** ham versiyonu incelemek isterseniz:
* 🔗 [**Game Actor (Orijinal Teslim Sayfası)**](https://jam.game.actor/game/208)

---

### ⚙️ Kurulum Adımları
1.  Yukarıdaki **Itch.io** butonuna tıklayın.
2.  Açılan sayfada **"Download"** butonuna basın.
3.  İndirdiğiniz ZIP dosyasını klasöre çıkarın.
4.  **KadimSakura.exe** ile oyunu başlatın.
---

## 📸 Ekran Görüntüleri (Gallery)

![Gameplay 1](https://img.itch.zone/aW1hZ2UvNDEzMTQ5Ny8yNDYxOTE5OS5wbmc=/794x1000/mM8Z9a.png)
